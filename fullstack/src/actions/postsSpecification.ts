...
export async function postCreateSpecification(data: FormData) {
  ...
  const specificationTypeId = parseInt(data.get('specificationTypeId') as string);

  const parsedData = formSpecification().formSchema.safeParse({ value, specificationTypeId, description});
